...
class Lottery 
{
    ...
    public function listAwards()
    {
        ...
        foreach ($query as $node) {
            ...
            $apportionment->type = trim(explode('-', $this->findRegex($node->nodeValue, '#(.+)#'))[0]);
            $apportionment->amount = trim($this->findRegex($node->nodeValue, '/([\d]+) aposta/', '0'));
            $apportionment->value = trim($this->findRegex($node->nodeValue, '/R\$ (.+)/', '0,00'));
    ...
    public function listCities()
    {
        ...
        foreach ($query as $node) {
            $city = new \stdClass;
            $city->name = trim($this->findRegex($node->nodeValue, '/((.+) - ([A-Z]{2}))/'));
            $city->amount = trim($this->findRegex($node->nodeValue, '/([\d]+) pessoa/', '0'));
            $city->numbers = trim($this->findRegex($node->nodeValue, '/([\d]+) acerto/', '0'));

            $matches = [];
            if (preg_match('/(.+) - ([A-Z]{2})/', $city->name, $matches)) {
                $cities[] = $city;
            }
        }
        return $cities;
